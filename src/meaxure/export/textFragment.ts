...
import { allSwats,allSwatsDic } from "./colors";
...
export function getTextFragment(artboard: Artboard, layer: Text, data: ArtboardData) {
    ...
    for (let frags of lines) {
        ...
        for (let fragment of frags) {
            ...
            // if(refred != null){
            //     subText.style.textColor = refred.referencingColor
            // }
    ...
    for (let text of sketch.find<Text>('Text', textGroup)) {
        // let  childMSImmutableColor = (<any>text).style.sketchObject.textStyle().attributes().MSAttributedStringColorAttribute;
        // let  childMsStyle = (<any>text).style.sketchObject.parentStyle().textStyle().attributes();
        // console.log(childMsStyle.MSAttributedStringColorAttribute.swatchID())
        // console.log((<any>text).style.sketchObject.textStyle())
        // console.log(nsDic);
    
        // console.log(nsDic.MSAttributedStringColorAttribute);
        // let fatherMSImmutableColor = (<any>text).style.sketchObject.textStyle().attributes().MSAttributedStringColorAttribute;
        // childMSImmutableColor.swatchID = fatherMSImmutableColor.swatchID()
...
function findSwatchWithID(id) { 
    let newId = new String(id).toString();
    let sw = allSwatsDic[newId];
    return sw ? sw : null;
    // for ( let i=0; i<allSwats.length;i++){ 
    //     // console.log(allSwats[i].referencingColor.swatchID());
    //     let jsStr = new String(allSwats[i].referencingColor.swatchID().toString()).toString();
    //     if (newId === jsStr) { return allSwats[i] } 
    //     if (allSwats.length - 1 === i) { return null } 
    // } 
}