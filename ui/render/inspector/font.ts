...
export function renderFont(layerData: LayerData): string {
    ...
    textColor2 = [
        '<div class="item" data-label="ARGB:">',
        '<div class="color">',
        colorItem2(layerData.color),
        '</div>',
        '</div>'
    ].join(''),
    keyText = [
        '<div class="item" data-label="Key :">',
        colorKey(layerData.color),
        '</div>'
    ].join(''),
    ...
    return propertyType('TYPEFACE', [fontFamily, textColor, textColor2, keyText, fontSize, spacing, content].join(''));
}
